export const bootstrap = () => {
    ...
    const canvas: any = document.getElementById('fluid');
  ...
  `);
    ...
    let curl: any;
    let pressure: any;
    // let ditheringTexture: any = createTextureAsync('../app/themes/flipp/dist/images/LDR_LLL1_0.png');

    // const blurProgram: any = new Program(blurVertexShader, blurShader);
    const copyProgram: any = new Program(baseVertexShader, copyShader);
    const clearProgram: any = new Program(baseVertexShader, clearShader);
    // const colorProgram: any = new Program(baseVertexShader, colorShader);
    const splatProgram: any = new Program(baseVertexShader, splatShader);
    const advectionProgram: any = new Program(baseVertexShader, advectionShader);
    const divergenceProgram: any = new Program(baseVertexShader, divergenceShader);
    const curlProgram: any = new Program(baseVertexShader, curlShader);
    const vorticityProgram: any = new Program(baseVertexShader, vorticityShader);
    const pressureProgram: any = new Program(baseVertexShader, pressureShader);
    const gradienSubtractProgram: any = new Program(baseVertexShader, gradientSubtractShader);
    ...
    function calcDeltaTime() {
        let now = Date.now();
        let dt = (now - lastUpdateTime) / 300; // time_sts
